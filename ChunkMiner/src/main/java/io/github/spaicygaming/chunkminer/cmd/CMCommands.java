	...
	public boolean onCommand(CommandSender sender, Command cmd, String alias, String[] args) {
		...
		// Length 3
			...
			if (args[0].equalsIgnoreCase("give")) {
				// Check permission
				if (!player.hasPermission(Const.PERM_GIVE)) {
					player.sendMessage(ChatUtil.c("noCmdPerms"));
					return false;
				}
				...
				if (target == null) {
					player.sendMessage(ChatUtil.c("targetOffline").replace("{target}", targetName));
					return false;
				}
				...
				if (target.getInventory().firstEmpty() == -1) {
					player.sendMessage(ChatUtil.c("targetFullInventory").replace("{target}", targetName));
					return false;
				}
				...
				// Messages
				player.sendMessage(ChatUtil.c("itemGived").replace("{target}", targetName)
						.replace("{amount}", String.valueOf(amount)));
				target.sendMessage(ChatUtil.c("itemReceivedOther").replace("{giver}", player.getName())
						.replace("{amount}", String.valueOf(amount)));
